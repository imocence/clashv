#! /usr/bin/env python
# -*- coding:utf-8 -*-
# Reference to https://github.com/poulp/zenipy
...
def password(text="", placeholder="", title="", width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, timeout=None):
    ...
    :rtype: str
    """
    dialog = ZEntryPassword(text, placeholder, title, width, height, timeout)
    dialog.run()
    return dialog.response


def zlist(columns, items, print_columns=None, text="", title="", width=DEFAULT_WIDTH, height=ZLIST_HEIGHT, timeout=None):
    """
...
def file_selection(multiple=False, directory=False, save=False, confirm_overwrite=False, filename=None,
                   title="", width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, timeout=None):
    """
    ...
    """
    dialog = ZFileSelection(multiple, directory, save, confirm_overwrite, filename, title, width, height, timeout)
    dialog.run()
    return dialog.response


def calendar(text="", day=None, month=None, title="", width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, timeout=None):
    """
    Display a calendar
    ...
    :rtype: tuple
    """
    dialog = ZCalendar(text, day, month, title, width, height, timeout)
    dialog.run()
    return dialog.response


def color_selection(show_palette=False, opacity_control=False, title="", width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, timeout=None):
    """
